...
export const CreateReportModal: React.FC<CreateReportModalProps> = ({
  ...
  // 5. Hours worked broken down by task type
  ...
  // Load projects list and populate fields
  useEffect(() => {
    if (isOpen) {
      projectsApi
        .getAll({ limit: 100 })
      ...
      if (!initialReport) {
        ...
        setPlansForNextWeek(['']);
        setBlockersList([{ description: '', isKeyIssue: false }]);
        setAchievementsList([{ description: '', isKeyAchievement: false }]);
        setHoursBreakdown({
          development: 25,
          testing: 5,
          meetings: 5,
        ...
        // Populate task-level table
        if (initialReport.tasks && initialReport.tasks.length > 0) {
          setTasks(initialReport.tasks);
        } else if (
          initialReport.tasksCompleted &&
          initialReport.tasksCompleted.length > 0
        ) {
          setTasks(
            initialReport.tasksCompleted.map((t) => ({
              taskName: t,
              ...
              plannedHours: 8,
              actualHours: 8,
              outputDeliverable: 'Completed deliverable',
            })),
          );
        }
        ...
        // Plans for next week
        setPlansForNextWeek(
          initialReport.plansForNextWeek && initialReport.plansForNextWeek.length
            ? initialReport.plansForNextWeek
            : [''],
        );

        // Blockers
        if (initialReport.blockersList && initialReport.blockersList.length > 0) {
          setBlockersList(initialReport.blockersList);
        } else if (initialReport.blockers) {
          setBlockersList([{ description: initialReport.blockers, isKeyIssue: true }]);
        } else {
          setBlockersList([{ description: '', isKeyIssue: false }]);
        }
        ...
        // Achievements
        if (
          initialReport.achievementsList &&
          initialReport.achievementsList.length > 0
        ) {
          setAchievementsList(initialReport.achievementsList);
        } else {
          setAchievementsList([{ description: '', isKeyAchievement: false }]);
        }

        // Hours breakdown
        if (initialReport.hoursBreakdown) {
          setHoursBreakdown(initialReport.hoursBreakdown);
        }

        setNotesOrLinks(initialReport.notesOrLinks || '');
      }
    }
  ...
  const handleSubmit = async (submitForReview = false) => {
    ...
    try {
      ...
      if (initialReport) {
        ...
        if (submitForReview) {
          saved = await reportsApi.submit(initialReport._id);
        ...
        if (submitForReview) {
          saved = await reportsApi.submit(saved._id);
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={initialReport ? 'Edit Personal Weekly Report' : 'New Personal Weekly Report'}
      description="Fixed standardized weekly report structure for engineering performance tracking"
      maxWidth="3xl"
    >
      <div className="space-y-6 max-h-[78vh] overflow-y-auto px-1 pr-2">
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="High-level summary of what was accomplished this week across the codebase and team goals..."
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-[11px] text-slate-500">
                Click the flame icon to flag the <span className="font-bold text-amber-700">Key Issue for the week</span>
              </p>
            </div>
            <button
              type="button"
        ...
        {/* 5. Achievements & Highlights with Key Achievement Flag */}
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <div>
              <label className="text-xs font-bold uppercase tracking-wider text-slate-800 flex items-center gap-1.5">
              ...
              <p className="text-[11px] text-slate-500">
                Click the star icon to flag the <span className="font-bold text-emerald-700">Key Achievement for the week</span>
              </p>
            </div>
            <button
              type="button"
            ...
            {achievementsList.map((item, idx) => (
              ...
              >
                ...
                <input
                  ...
                  placeholder={`Major highlight or win #${idx + 1}`}
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 pt-4 border-t border-slate-100">
          ...
          <div className="flex items-center gap-2">
            <Button
              ...
              variant="secondary"
              size="sm"
              isLoading={loading && !submitDirectly}
              onClick={() => {
                setSubmitDirectly(false);
                handleSubmit(false);
              }}
              icon={<Save className="h-3.5 w-3.5" />}
            >
              {initialReport ? 'Save Changes' : 'Save as Draft'}
            </Button>
